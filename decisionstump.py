...
def minLabelErrorOfHypothesisAndNegation(data, h):
    ...
    posError = sum(y == -1 for (x, y) in posData) + sum(
        y == 1 for (x, y) in negData)
    negError = sum(y == 1 for (x, y) in posData) + sum(
        y == -1 for (x, y) in negData)
    return min(posError, negError) / len(data)


def bestThreshold(data, index, errorFunction):
    """Compute best threshold for a given feature. Returns (threshold,
    error) """
    ...
    errors = [(threshold, errorFunction(data, makeThreshold(threshold))) for
...
def buildDecisionStump(drawExample, errorFunction=defaultError, debug=True):
    ...
    bestThresholds = [(i,) + bestThreshold(data, i, errorFunction) for i in
                      range(len(data[0][0]))]
