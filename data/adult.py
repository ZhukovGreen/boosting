...
featureNames = (
    ('age', 'sex') + employers + ('education',) + maritals + occupations +
...
def processLine(line):
    ...
    point = ([int(age), 0 if sex == 'Female' else 1] + vectorize(employer,
                                                                 employers) + [
                 int(education)] +
             vectorize(marital, maritals) + vectorize(occupation,
                                                      occupations) + vectorize(
        race, races) +
             [int(capital_gain), int(capital_loss),
              int(hr_per_week)] + vectorize(country, countries))
