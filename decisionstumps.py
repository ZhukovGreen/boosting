...
def dataToDistribution(data):
    ...
    return [float(allLabels.count(aLabel)) / numEntries for aLabel in
            possibleLabels]
...
def gain(data, index, threshold):
    ...
    dataSubsets = [
        [(point, label) for (point, label) in data if
         point[index] >= threshold],
        [(point, label) for (point, label) in data if
         point[index] < threshold]]
...
def buildDecisionStump(drawExample):
    ...
    bestThresholds = [(i, bestThreshold(data, i)) for i in
                      range(len(data[0][0]))]
